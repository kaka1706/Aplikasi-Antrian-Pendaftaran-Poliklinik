...
<div class="card">
    <div class="card-header">
        <h5 class="card-title mb-0" style="color:#133E87;">Edit Klinik</h5>
    </div>
    <div class="card-body p-4">
        <form action="{{ route('clinics.update', $clinic) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('address') is-invalid @enderror" id="address" name="address" rows="3" required>{{ old('address', $clinic->address) }}</textarea>
                @error('address')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', $clinic->description) }}</textarea>
                @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
